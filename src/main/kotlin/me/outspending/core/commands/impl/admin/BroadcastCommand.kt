...
class BroadcastCommand {
    fun onCommand(sender: CommandSender) {
        val broadcasts = core.broadcastManager.broadcasts.joinToString("") {
        ...
        sender.sendMessage(("<gray>Broadcasts:\n" +
                "<gray><i>(( Protected broadcasts can't be cleared ))<reset>\n" +
                broadcasts).parse(true))
    }
    ...
    fun add(sender: CommandSender, customId: String, message: String) {
    ...
    fun remove(sender: CommandSender, customId: String) {
        core.broadcastsConfig.removeValue(customId)
        core.broadcastsConfig.save()
        core.broadcastManager.removeBroadcastWithCustomId(customId)
    ...
    fun reload(sender: CommandSender) {
        ...
        val time = measureTime {
            core.broadcastsConfig.reload()
            core.broadcastManager.clearBroadcasts()
            core.broadcastManager.registerFromConfig(core.broadcastsConfig)
        }
        sender.sendMessage("<gray>Broadcasts reloaded in <main>$time".parse(true))
        val fromConfig = core.broadcastsConfig.getBroadcasts().map { it.key }.joinToString("") {
            "<gray>${it}\n"
        }
    ...
    fun send(sender: CommandSender, customId: String) {
    ...
    fun message(sender: CommandSender, @Catcher message: String) {
