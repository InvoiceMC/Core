...
import me.outspending.core.broadcaster.BroadcastHandler.broadcastManager
import me.outspending.core.broadcaster.BroadcastHandler.broadcastsConfig
...
class BroadcastCommand {
    fun onCommand(sender: CommandSender) {
        val broadcasts =
            broadcastManager.broadcasts.joinToString("") {
    ...
    fun add(
        ...
        if (broadcastManager.broadcasts.any { it.customId.equals(customId, ignoreCase = true) }) {
            sender.sendMessage("<red>Broadcast with custom id already exists".parse(true))
            return
        }
        broadcastManager.addBroadcast(customId, message)
    ...
    fun send(
        ...
        val broadcast = broadcastManager.broadcasts.find { it.customId.equals(customId, ignoreCase = true) }
        if (broadcast == null) {
